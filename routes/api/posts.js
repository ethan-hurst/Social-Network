...
router.get('/', (req, res) => {
  Post.find()
    ...
    .catch((err) => res.status(404).json({
      err,
      nopostsfound: 'There are no posts found!'
    }))
})

/**
 * POST Route for /api/posts/like/:id
 * @desc Like Posts
 * @access Private
 */
...
/**
 * POST Route for /api/posts/unlike/:id
 * @desc Like Posts
 * @access Private
 */
        ...
        .then(post => {
          if (post.user.toString() !== req.user.id) {
            return res.status(401).json({
              notauthorized: 'You\'re not authorized to delete that!'
            })
          }
          post.remove()
            .then(() => res.json({
          ...
          err,
          nopostfound: 'Could not Delete post! Unable to locate a post with that ID!'
        }))
    })
})
...
/**
 * DELETE Route for /api/posts/comment/:id/:comment_id
    ...
    .then(post => {
      if (post.comments.filter(comment => comment._id.toString() === req.params.comment_id).length === 0) {
        return res.status(404).json({
          nocommentexists: 'No comment exists!'
        })
      }
